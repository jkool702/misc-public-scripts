...
UEFIDracutUpdate() { 
    ...
    # 
    # NOTE: because this sets up the ROOT device in the kernel commandline, the root filesystem mount should NOT be listed in /etc/fstab nor (if using LUKS) in /etc/crypttab. 
    #      However, any other devices/mounts (e.g., for a seperate home directory) should still be listed in /etc/{fs,crypt}tab, unless you are using ZFS (then zfs handles it)
    #       if you do have the root filesystem them listed listed in /etc/{fs,crypt}tab it can, in some instances, lead to issues 
    ...
    if (( ${useHardcodedZFSInfoFlag} == 1 )); then
        ...
        bootDisk='/dev/disk/by-id/<type>-<make>-<model>-<serial>-part<N>'
        luksUUID='luks-<luksUUID>'
        #luksUUID="luks-$(cryptsetup luksUUID /dev/disk/by-id/nvme-<make>_<model>_<serial>-part<N>)"
...
inst_kern="${1}"
[[ -d "/usr/lib/modules/${inst_kern}" ]] || inst_kern="$(/bin/ls -1 /usr/lib/modules | /bin/sort -V | /bin/tail -n 1)"

if [[ "$(realpath /etc/alternatives/ld)" == '/usr/bin/mold' ]]; then
...
# build all dkms modules for the new kernel. explicitly build zfs.
/sbin/dkms install --kernelver "${inst_kern}" -m zfs -v $(/bin/find /usr/src -maxdepth 1 -type d -name 'zfs*'| /bin/sort -rVu | /bin/head -n 1 | /bin/sed -E s/'^.*-([^\-]*)$'/'\1'/)
/sbin/dkms autoinstall --kernelver "${inst_kern}" 2>/dev/null 1>/dev/null
/bin/sleep 1 

# start building nvidia kernel modules
/bin/rpm -qa | /bin/grep -q 'akmod' && /bin/systemctl restart --no-block akmods@${inst_kern}.service 
/bin/sleep 1 
...
timeout=30
while (( ${timeout} > 0 )); do
        /sbin/modinfo -k "${inst_kern}" zfs 1>/dev/null 2>/dev/null && break || /sbin/depmod -A
        sleep 1
        ((timeout--))
done

(( ${timeout} == 0 )) && /bin/echo -e "\n\n||----------------------------------------------------------------------|| \n* WARNING: THE ZFS KERNEL MODULES DID NOT BUILD CORRECTLY \nMANUALLY BUILD AND SIGN THESE AND THEN REGENERATE YOUR INITRAMFS \n||----------------------------------------------------------------------|| \n\n" >&2

/bin/systemctl daemon-reload 
/sbin/depmod -a 

/bin/systemctl start my-modsign@${inst_kern}.service

/bin/ln -s /lib/modules/${inst_kern}/vmlinux /lib/modules/${inst_kern}/build/vmlinux
${restoreLD} && ln -sf /usr/bin/ld.mold /etc/alternatives/ld
